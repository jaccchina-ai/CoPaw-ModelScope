...
class EveningAnalyzerV4:
    ...
    def _apply_p3_risk_adjustment(self, stock_data, score, details):
        ...
        try:
            # 调用风控引擎评估个股风险
            stock_risk = self.risk_engine.assess_stock_risk(
                stock_data, 
                details.get('dragon_tiger_data')
            )
            
            # 根据风险等级调整分数
            risk_level = stock_risk.get('risk_level', '低')
            risk_score = stock_risk.get('total_risk_score', 0)
            
            if risk_level == '高':
                # 高风险股票大幅降权
                adjusted_score *= 0.5
                risk_flags.append(f"高风险股票(风险分{risk_score}×0.5)")
                self.risk_stats['warnings'].append(
                    f"{stock_data.get('name', '')}: {stock_risk.get('recommendation', '')}"
                )
            elif risk_level == '中':
                # 中风险股票适度降权
                adjusted_score *= 0.8
                risk_flags.append(f"中风险股票(风险分{risk_score}×0.8)")
            
            # 添加详细风险提示
            for warning in stock_risk.get('warnings', []):
    ...
    def run_full_analysis(self):
        ...
        # ★P4新增：情绪周期细化分析
        print(f"\n【P4分析】情绪周期细化...")
        self.emotion_stage = self.emotion_analyzer.analyze_emotion(self.date)
